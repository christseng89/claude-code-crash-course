...
1. **Consumer Lag 告警（Prometheus Rules）**
...
6. **階段三：金絲雀發布（Canary Release，2-4 週）**
...
**強烈建議（Should Have）**
7. ✅ Saga Pattern（跨服務一致性）
8. ✅ 版本化交易模型（並發修改處理）
9. ✅ 分散式追蹤（correlationId + traceId）
10. ✅ DLQ + 告警（錯誤處理）
11. ✅ 漸進式遷移（Shadow → Canary → Full）
12. ✅ 回滾計畫（Go-live 保險）

**可選（Nice to Have）**
13. ⭕ Event Sourcing（完整事件歷史）
14. ⭕ CQRS（讀寫分離）
15. ⭕ CDC（Change Data Capture）
16. ⭕ Kafka Streams（實時處理）
...
>         {"name": "customerName", "type": ["null", "string"], "default": null}  // 新增欄位（可選）
>       ]
>     }},
>     {"name": "metadata", "type": ["null", "map"], "default": null}  // 擴展性預留
>   ]
> }
> ```
>
